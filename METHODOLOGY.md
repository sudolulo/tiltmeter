...
recommended storage format; Bhushan et al., LoC Recommended Formats). A database
server would add credentials and operational surface while our write rate (one
single-writer batch per few hours) needs none of it. Content addressing makes
deduplication and integrity the same mechanism: the storage key *is* the checksum
(the design underlying git and content-addressable stores generally). The hash
chain applies the certificate-transparency/append-only-log idea (Laurie et al.,
RFC 6962) at dataset scale: history cannot be rewritten without breaking a chain
whose heads can be recorded elsewhere.

**Alternatives considered**: PostgreSQL — operationally heavier, artifact is no
longer a file, solves no present problem; revisit only if concurrent writers ever
exist. DuckDB — analytical engine, not a live collection store. Per-article flat
files + manifest — auditable but slow to query and easy to partially copy
incorrectly. zstd compression — slightly better ratio than zlib but adds a
dependency; zlib is stdlib and sufficient (~2× on-disk reduction measured on the
real corpus, on top of story-level dedup). Float16 embeddings — halves cache size
but perturbs downstream numbers; reproducibility outranks bytes.

**Failure modes**: SQLite foreign keys are per-connection and a tamperer's
connection wouldn't enable them — which is why `audit` never relies on them and
re-hashes everything instead. The chain proves order and integrity of what was
recorded, not that collection itself was honest — that is what publishing chain
heads externally (e.g., committing them alongside releases) and third-party
re-collection are for. Timestamps in chain entries are self-reported; external
anchoring bounds, but does not eliminate, backdating within the gap between
published heads.
